  ...
  return (
    <>
      <ReactFullpage
        scrollingSpeed={1000}
        onLeave={handleLeave}
        anchors={["issue8", "issue7", "issue6", "issue5", "issue4", "issue3", "issue2", "issue1"]}
        render={() => {
          return (
      ...
      />
      <ul id="menu" className="hidden md:block md:fixed right-4 bottom-4 text-sm leading-6">
        {[
          ...
          <li key={item.anchor} data-menuanchor={item.anchor}>
            <a href={`#${item.anchor}`} className={`hover:underline ${activeIssue===item.anchor?"font-bold":""}`}>{item.label}</a>
          </li>
        ))}
      </ul>
    </>
